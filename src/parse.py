    ...
    def _attach(self, item: Item, position: int) -> None:
        ...
        if item.rule.lhs in self.cols[mid]._next_symbol_index:
            ...
            for idx in self.cols[mid]._next_symbol_index[item.rule.lhs]:
                ...
                # for addedItem in self.cols[position]._index.copy():
                #     if new_item.__eq__(addedItem):
                #         if new_item.weight < addedItem.weight:
                #             rm_and_pushed = True
                #             idx = self.cols[position]._index[addedItem]
                #             self.cols[position].remove_and_push(idx, new_item)
                # # # If added in the previous step, will not push the new_item
                # if new_item not in self.cols[position]._index and not rm_and_pushed:
                self.cols[position].push(new_item)
                # if (new_item.rule.lhs == self.grammar.start_symbol and new_item.next_symbol() is None and new_item.start_position == 0):
                #     if new_item.weight < self.minWeight:
                #         self.rootItem.append(new_item)
...
class Agenda:
    ...
    # def remove_and_push(self, index: int, item: Item) -> None:
    #     rm = self._items.pop(index)
    #     del self._index[rm]
    #     self._next_symbol_index[rm.next_symbol()].discard(index)
    #     self._items.insert(index, item)
    #     self._index[item] = index  
    #     self._next_symbol_index[rm.next_symbol()].add(index)
